...
# This code to show specific imaage for each category
    ...
    if 'username' not in login_session:
        return render_template('publiccategory.html',
                               category=category, items=items, img=img)
    else:
...
def showrecipe(category_id, recipe_id):
    ...
    if 'username' not in login_session:
        return render_template('publicrecipe.html', category=category,
                               item=item, ingredients=ingredients,
                               instructions=instructions)
    else:
        return render_template('recipe.html', category=category,
                               item=item, ingredients=ingredients,
                               instructions=instructions)
...
def newrecipe(category_id):
    ...
    if request.method == 'POST':
        nutritions1 = [Nutritions(energy=request.form['energy'],
                                  calories=request.form['calories'],
                                  fat=request.form['fat'],
                                  saturatedFat=request.form['s-fat'],
                                  carbohydrate=request.form['carbs'],
                                  sugar=request.form['sugar'],
                                  dietaryFiber=request.form['fibers'],
        ...
        newItem = Item(title=request.form['name'],
                       ...
                       preparingTime=request.form['p-time'],
                       cookingTime=request.form['c-time'],
...
@app.route('/catalog/<int:category_id>/recipe/<int:recipe_id>/edit/',
           methods=['GET', 'POST'])
def editrecipe(category_id, recipe_id):
    editedItem = session.query(Item).filter_by(id=recipe_id).one()
    nutritions = session.query(Nutritions).filter_by(
    ...
    if editedItem.user_id != login_session['user_id']:
        ...
        Please create your own recipe in order to edit.');
         window.location.href = '/catalog/"""+str(category_id)+"/recipes/';}
...
@app.route('/catalog/<int:category_id>/recipe/<int:recipe_id>/delete/',
           methods=['GET', 'POST'])
def deleterecipe(category_id, recipe_id):
    ...
    if deletedItem.user_id != login_session['user_id']:
        return '''<script>function myFunction() {
            alert('You are not authorized to delete this recipe. Please create
            your own recipe in order to delete.');window.location.href =
            '/catalog/"""+str(category_id)+"/recipes/'}</script>
